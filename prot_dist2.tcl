...
for {set i 1} {$i <= 70 } {incr i} {
	...
	set v1a [vecnorm [vecsub $end1_cent $mid1_cent]]
        ...
        $trimer move [eval "trans center {$memb_mid_pt1_cent} offset {$pt1} axis $cross1 $ang1 rad"]
        $trimer delete
	$mid1 delete
	$end1 delete
	$memb_mid_pt1 delete
...
for {set i 1} {$i <= 70 } {incr i} {
	...
	set v1a [vecnorm [vecsub $end2_cent $mid2_cent]]
        ...
        set ang1 [expr acos(max(-1.0,min(1.0,[vecdot $v1a $v1b])))]
        set cross1 "{[veccross $v1a $v1b]}"
        $pentamer move [eval "trans center {$memb_mid_pt2_cent} offset {$pt2} axis $cross1 $ang1 rad"]
        $pentamer delete
	$mid2 delete
	$end2 delete
	$memb_mid_pt2 delete
...
for {set i 1} {$i <= 560 } {incr i} {
	...
	set v1a [vecnorm [vecsub $end3_cent $mid3_cent]]
        ...
        set ang1 [expr acos(max(-1.0,min(1.0,[vecdot $v1a $v1b])))]
        set cross1 "{[veccross $v1a $v1b]}"
        $dimer move [eval "trans center {$memb_mid_pt3_cent} offset {$pt3} axis $cross1 $ang1 rad"]
        $dimer delete
	$mid3 delete
	$end3 delete
	$memb_mid_pt3 delete
...
[atomselect top all] writepdb S_E_M_dist_700b.pdb
[atomselect top all] writepsf S_E_M_dist_700b.psf
